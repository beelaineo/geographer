...
type ReclusInterviewDetailPageProps = {
  params: Promise<{ interviewSlug: string }>;
};

export async function generateMetadata({ params }: ReclusInterviewDetailPageProps): Promise<Metadata> {
...
export default async function ReclusInterviewDetailPage({ params }: ReclusInterviewDetailPageProps) {
  const { isEnabled } = await draftMode();
  const { interviewSlug } = await params;
